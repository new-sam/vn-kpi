...
CREATE TABLE IF NOT EXISTS matches (
  ...
  stage TEXT NOT NULL CHECK (stage IN ('lead', 'submitted', 'interview', 'verbal', 'confirmed', 'dropped')),
...
-- 기존 테이블에 note(비고) 컬럼 추가 (idempotent)
ALTER TABLE matches ADD COLUMN IF NOT EXISTS note TEXT;

-- stage CHECK 제약에 'verbal'(구두채용) 추가 (idempotent)
ALTER TABLE matches DROP CONSTRAINT IF EXISTS matches_stage_check;
DO $$ BEGIN
  ALTER TABLE matches ADD CONSTRAINT matches_stage_check CHECK (stage IN ('lead','submitted','interview','verbal','confirmed','dropped'));
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- 단계 변경 시점 컬럼 (월별 집계는 등록일이 아니라 이 시점 기준)
ALTER TABLE matches ADD COLUMN IF NOT EXISTS verbal_at DATE;
ALTER TABLE matches ADD COLUMN IF NOT EXISTS confirmed_at DATE;
-- 기존 행 백필: verbal/confirmed 단계의 등록일을 그대로 사용
UPDATE matches SET verbal_at    = date WHERE stage = 'verbal'    AND verbal_at    IS NULL;
UPDATE matches SET confirmed_at = date WHERE stage = 'confirmed' AND confirmed_at IS NULL;
